/*
  ...
  Usage : GET /api/aelf?office=laudes&y=2025&m=05&d=02
...
export default async function handler(req, res) {
  ...
  try {
    const upstream = await fetch(url, {
      headers: { 'Accept': 'application/json', 'User-Agent': 'prionsenligne.fr/1.0' },
    });

    const text = await upstream.text();

    if (!upstream.ok) {
      console.error(`[aelf-proxy] AELF returned ${upstream.status} for ${url}`);
